window.VI3DR_TRANSLATIONS = {
  en: {
    title: "VI3DR Control Room",
    actions: {
      skip_to_content: "Skip to content",
      close: "Close",
      choose_model: "Choose model",
      choose_database: "Choose JSON",
      send_debug_response: "Send info-response",
    },
    labels: {
      backend: "Backend",
      session: "Session",
      live_preview: "Live Preview",
      phone_preview: "Phone Preview",
      transport: "Transport",
      port: "Port",
      mdns_ip: "mDNS / LAN IP",
      language: "Language",
      client_ip: "Client IP",
      fps: "FPS",
      compression: "Compression",
      detection: "Detection",
      model: "Model",
      model_status: "Status",
      knowledge_database: "Knowledge DB",
      database_file: "File",
      database_status: "Status",
      detected_object: "Detected Object",
      probability: "Probability",
      system_details: "System details",
      debug: "Debug",
      debug_message: "Message",
    },
    messages: {
      no_frame: "No frame",
      no_detection: "No detection",
      preview_alt: "Live camera preview from the connected phone.",
      waiting_for_phone: "Waiting for a phone connection",
      waiting_for_phone_detail: "Waiting for a phone connection.",
      no_session_details: "No session details available.",
      backend_waiting: "Waiting for desktop runtime data.",
      model_waiting: "Waiting for model status.",
      database_waiting: "Waiting for database status.",
      debug_message_placeholder: "Text sent as info-response",
    },
    status: {
      booting: "BOOTING",
      starting: "STARTING",
      running: "RUNNING",
      stopping: "STOPPING",
      stopped: "STOPPED",
      error: "ERROR",
      unknown: "UNKNOWN",
      idle: "IDLE",
      connected: "CONNECTED",
      disconnected: "DISCONNECTED",
    },
    model_status: {
      booting: "BOOTING",
      loading: "LOADING",
      ready: "READY",
      disabled: "DISABLED",
      error: "ERROR",
      unknown: "UNKNOWN",
    },
    model_message: {
      booting: "Detection is starting.",
      loading: "Loading detection model.",
      ready: "Detection model is ready.",
      disabled: "Detection is disabled.",
      error: "Detection model error.",
      unknown: "No model status available.",
    },
    database_message: {
      booting: "Knowledge database is starting.",
      loading: "Loading knowledge database.",
      ready: "Knowledge database is ready.",
      error: "Knowledge database error.",
      unknown: "No database status available.",
    },
    capture: {
      connected: "Phone connected.",
      disconnected: "Session closed.",
      idle: "Waiting for a phone connection.",
      error: "Capture session error.",
    },
    errors: {
      qwebchannel_not_initialized: "QWebChannel is not initialized.",
      backend_error: "Backend error.",
    },
    meta: {
      frame_dimensions: "{width} × {height}",
    },
  },
  pl: {
    title: "VI3DR Control Room",
    actions: {
      skip_to_content: "Przejdź do treści",
      close: "Zamknij",
      choose_model: "Wybierz model",
      choose_database: "Wybierz JSON",
      send_debug_response: "Wyślij info-response",
    },
    labels: {
      backend: "Backend",
      session: "Sesja",
      live_preview: "Podgląd na żywo",
      phone_preview: "Podgląd z telefonu",
      transport: "Transport",
      port: "Port",
      mdns_ip: "mDNS / LAN IP",
      language: "Język",
      client_ip: "IP klienta",
      fps: "FPS",
      compression: "Kompresja",
      detection: "Detekcja",
      model: "Model",
      model_status: "Status",
      knowledge_database: "Baza wiedzy",
      database_file: "Plik",
      database_status: "Status",
      detected_object: "Wykryty obiekt",
      probability: "Prawdopodobieństwo",
      system_details: "Szczegóły systemu",
      debug: "Debug",
      debug_message: "Wiadomość",
    },
    messages: {
      no_frame: "Brak klatki",
      no_detection: "Brak detekcji",
      preview_alt: "Podgląd obrazu na żywo z połączonego telefonu.",
      waiting_for_phone: "Oczekiwanie na połączenie telefonu",
      waiting_for_phone_detail: "Oczekiwanie na połączenie telefonu.",
      no_session_details: "Brak szczegółów sesji.",
      backend_waiting: "Oczekiwanie na dane z warstwy desktopowej.",
      model_waiting: "Oczekiwanie na status modelu.",
      database_waiting: "Oczekiwanie na status bazy wiedzy.",
      debug_message_placeholder: "Tekst wysyłany jako info-response",
    },
    status: {
      booting: "URUCHAMIANIE",
      starting: "START",
      running: "AKTYWNY",
      stopping: "ZAMYKANIE",
      stopped: "ZATRZYMANY",
      error: "BŁĄD",
      unknown: "NIEZNANY",
      idle: "BEZCZYNNA",
      connected: "POŁĄCZONA",
      disconnected: "ROZŁĄCZONA",
    },
    model_status: {
      booting: "URUCHAMIANIE",
      loading: "WCZYTYWANIE",
      ready: "GOTOWY",
      disabled: "WYŁĄCZONY",
      error: "BŁĄD",
      unknown: "NIEZNANY",
    },
    model_message: {
      booting: "Detekcja się uruchamia.",
      loading: "Wczytywanie modelu detekcji.",
      ready: "Model detekcji jest gotowy.",
      disabled: "Detekcja jest wyłączona.",
      error: "Błąd modelu detekcji.",
      unknown: "Brak statusu modelu.",
    },
    database_message: {
      booting: "Baza wiedzy się uruchamia.",
      loading: "Wczytywanie bazy wiedzy.",
      ready: "Baza wiedzy jest gotowa.",
      error: "Błąd bazy wiedzy.",
      unknown: "Brak statusu bazy wiedzy.",
    },
    capture: {
      connected: "Telefon połączony.",
      disconnected: "Sesja zamknięta.",
      idle: "Oczekiwanie na połączenie telefonu.",
      error: "Błąd sesji przechwytywania.",
    },
    errors: {
      qwebchannel_not_initialized: "QWebChannel nie został zainicjalizowany.",
      backend_error: "Błąd backendu.",
    },
    meta: {
      frame_dimensions: "{width} × {height}",
    },
  },
};
