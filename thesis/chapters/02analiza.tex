...
% ! Umieść tabelę w odpowiednim miejscu! W razie potrzeby skróć tekst w kolumnach, aby zmieściła się na stronie. W ostateczności można również zmniejszyć czcionkę lub zmienić szerokość kolumny.
...
\begin{table}[ht]
	...
	\begin{tabularx}{\textwidth}{
		...
		Wysoka skuteczność i dokładna określanie położenia obiektów                                                 &
...
% ! Sprawdzić dokładnie poprawność!
...
Kolejnym ważnym aspektem wersji 26 jest fakt rezygnacji z użycia DFL (ang. \english{Distribution Focal Loss}) w trakcie regresji ramek, co pozwala zmniejszyć złożoność głowicy decyzyjnej i usunąć ograniczenia wynikające z dyskretnej reprezentacji odległości. Do procesu uczenia wprowadzono mechanizmy Progressive Loss oraz STAL (ang. \english{Small-Target-Aware Label Assignment}), poprawiający przypisanie przykładów pozytywnych dla małych obiektów. W procesie optymalizacji wykorzystano optymalizator MuSGD \cite{bib:jocher2026ultralyticsyolo26}.

% ! Sprawdzić dokładnie poprawność i ułożyć poprawnie na stronie!
...
Ze względu na specyfikę systemu, spośród dostępnych wariantów YOLO26 zdecydowano się na wykorzystanie podstawowego, zajmującego się detekcją obiektów na obrazie. Wynik zawierający klasę obiektu, wartość pewności oraz jego prostokątna ramka ograniczająca dostarcza wystarczającą liczbę informacji o obiekcie do realizacji zadania systemu. Segmentacja umożliwiłaby kompletne wyodrębnianie obiektu z tła, ale wymagałoby to przygotowania masek segmentacyjnych dla zbioru uczącego, a także zwiększyłaby złożoność obliczeniową modelu. Ciekawym kierunkiem rozwoju może być wykorzystanie wariantu z ramkami obróconymi, wprowadzonym w najnowszej wersji YOLO, aby ograniczyć udział tła w ramce detekcji.

\subsection{Warianty ze względu na skalę modelu}

Każdy wariant YOLO26 jest dostępny w kilku skalach różniących się od siebie liczbą parametrów oraz kosztem obliczeniowym. Wybór odpowiedniego wariantu powinien być oparty na wymaganiach systemu oraz ograniczeniach zasobów sprzętowych urządzenia końcowego. Autorzy modelu detekcyjnego przewidzieli pięć wariantów: \texttt{n} (\english{nano}), \texttt{s} (\english{small}), \texttt{m} (\english{medium}), \texttt{l} (\english{large}) oraz \texttt{x} (\english{extra large}). W referencyjnych wynikach każdy kolejny wariant osiąga wyższe wartości $mAP50\text{-}90$, wymagając jednocześnie większej ilości parametrów i operacji \cite{bib:ultralyticsYOLO26}.
...
Wyniki, które zostały przedstawione w tabeli~\ref{tab:warianty_YOLO} pokazują kompromis, typowy dla tej rodziny modeli. Wzrost rozmiaru modelu skorelowany jest ze wzrostem wartości $mAP50\text{-}95$, liczbą parametrów, liczbą wykonywanych operacji, oraz czasem wykonania. Największy wzrost skuteczności obserwujemy przy przejściu z modelu YOLO26n do YOLO26s, wynoszącym $7,7$ punktu $mAP$ pierwszego z nich, a dalsze zwiększanie rozmiaru przynosi coraz mniejsze przyrosty względem rosnącego kosztu.

Wartości czasowe służą porównaniu wariantów w jednakowych warunkach i nie mogą być utożsamiane z wartością pewną czasu wykonania. Ten zależy od sprzętu, na którym uruchamiany jest model, oraz innych czynników wynikających z implementacji danego systemu.

\subsection{Dobór wariantów modelu}

Do dalszej analizy przyjęto najnowszą wersję modeli rozwijanych przez Ultralytics -- YOLO26. Ta wersja wprowadza rozwiązania istotne z punktu projektowanego systemu, przede wszystkim rozwojem podejścia do detekcji end-to-end bez etapu NMS i uproszczoną regresję ramek. Te zmiany ograniczają złożoność głowicy modelu i zakres przetwarzania, jaki był wykonywany po zakończeniu pracy sieci \cite{bib:jocher2026ultralyticsyolo26}.
...
Wariant \texttt{n} charakteryzuje się najmniejszą liczbą parametrów i najmniejszym kosztem wykonywanych obliczeń, przez co czyni go idealnym Kandydatem dla jednostek o ograniczonych zasobach. Wariant \texttt{s} stanowi kolejny poziom złożoności sieci neuronowej, a wyniki referencyjne wskazują na wyższą wartość $mAP$. Uwzględnienie obu wariantów pozwoli przeanalizować wpływ poszczególnych wariantów i kombinacji parametrów na wydajność i dokładność detekcji.

\section{Wykorzystanie danych wizualnych i przestrzennych w rozpoznawaniu obiektów}

Rozpoznawanie obiektów nie musi opierać się jedynie na klasycznym obrazie RGB. Współczesne urządzenia mogą oferować dostęp do danych oferujących opis geometrii obserwowanej sceny, przedstawianej w postaci chmur punktów, czy map głębi. Dane przestrzenne mogą bardzo dobrze uzupełniać informacje o kolorze, teksturze, i strukturze obrazu rejestrowanego przez kamerę. Każdy z wymienionych sposobów reprezentacji danych wiąże się jednak z odmiennymi ograniczeniami i wymaganiami, które należy rozpatrywać w kontekście konkretnego zastosowania.

\subsection{Obraz RGB}

Obrazy kolorowe, najczęściej reprezentowane w przestrzeni RGB (ang. \textit{red-green-blue}), stanowią obecnie jedną z najpowszechniejszych form reprezentacji danych w dziedzinie wizji komputerowej \cite{bib:wang2023imagesampling}. Kamery obecnie znajdujemy w większości dziedzin życia: monitoringu, medycynie, przemyśle, motoryzacji czy w urządzeniach osobistych. Istotne jest więc określenie, jakie informacje zawarte w zarejestrowanym obrazie mogą zostać wykorzystane do rozpoznawania obiektów 3D.

Obraz zarejestrowany przez kamerę jest reprezentacją trójwymiarowej sceny na płaszczyźnie dwuwymiarowej. Pomimo usunięcia jednego wymiaru przestrzennego, z pojedynczej klatki wyodrębnić można bardzo dużo cech użytecznych podczas analizy obiektów, m.in. o kolorze, teksturze, krawędziach, zmianach jasności czy cieniach. Te informacje mogą z powodzeniem zostać wykorzystane przez modele uczenia maszynowego do wyznaczenia charakterystycznych cech przedstawionych obiektów. Obrazy RGB stanowią również podstawową formę danych wejściowych dla wielu CNN oraz detektorów obiektów, w tym rodziny YOLO (\ref{s:yolo}).
...
Po przetworzeniu obraz RGB może być reprezentowany w formacie macierzy trójwymiarowej o wymiarach

\begin{equation}
	M \times N \times 3,
\end{equation}

gdzie $M$ i $N$ to liczba wierszy i kolumn obrazu, a trzeci wymiar odpowiada kanałom barwnym: czerwonemu, zielonemu i niebieskiemu. Wartość pojedynczego piksela jest więc krotką trzech wartości, które najczęściej występują w kombinacji

\begin{equation}
	I(i,j) = (R, G, B).
\end{equation}

Pojedynczy piksel zawiera jedynie informację o wartościach trzech składowych koloru, ale zależności przestrzenne pomiędzy wieloma pikselami pozwalają odwzorować strukturę obserwowanej sceny. Lokalne zmiany wartości pikseli pozwalają m.in. wyznaczać gradienty obrazu, które mogą posłużyć do wykrywania krawędzi i innych strukturalnie ważnych elementów \cite{bib:sobel2014isotropic}.
Jednakże, pomimo dużej ilości danych dostępnych do odczytu w samym obrazie, reprezentacja ta ma również istotne ograniczenia. Przede wszystkim, wygląd obserwowanego obiektu zależy znacząco od warunków oświetleniowych i stopnia zasłonięcia. Zmiana tych warunków może doprowadzić do różnic w kolorach, kontraście czy widoczności jego krawędzi, tym samym wpływając na zdolność jego rozpoznania \cite{bib:szeliski2021cvalgorithms}. Brak informacji o odległości punktu od kamery jest również istotnym ograniczeniem.
Projekcja dwuwymiarowa skutkuje utratą części informacji przestrzennej, przez co różne obiekty -- często zasłonięte -- mogą prowadzić do bardzo podobnych obrazów. Istnieją modele estymujące głębię (np. YOLO26*-depth \ref{ssec:warianty_zadanie}), jednak otrzymane z nich wartości nie stanowią pomiaru odległości, a ich dokładność zależy od wielu czynników. Z tego względu uzupełnienie obrazu o informację dot. odległości punktów od kamery wydaje się przydatne podczas procesu rozpoznania brył trójwymiarowych.
...
Najprostsza ze względu na wymaganą infrastrukturę jest estymacja głębi z pojedynczego obrazu RGB. Należy jednak przy tym mieć na uwadze, że nie jest do dokładny pomiar, a jedynie przybliżenie wartości odległości. Najmniej wymagającym pod względem sprzętowym jest estymacja głębi z pojedynczego obrazu, określana jako \english{monocular depth estimation}. To podejście opiera się na wyznaczeniu struktury przestrzennej na podstawie cech obrazu. Przykładami tej techniki są: rozwiązanie zaproponowane przez Ranftla et al. oraz Monodepth2 opracowane przez zespół Godarda. Te metody umożliwiają uzyskać mapę głębi bez stosowania dodatkowych urządzeń. Należy jednak pamiętać, że wynik nie jest wartością rzeczywistą, a jedynie estymacją.

Kolejno metodą jest stereowizja, wykorzystująca dwa obrazy zarejestrowane z różnych położeń. Znając wzajemne położenie kamer i analizując dysparcję możliwe jest określenie różnicy położenia odpowiadających sobie punktów na obrazach. Technologia stereoskopowa znalazła zastosowanie w urządzeniach konsumenckich. W 2010 roku firma Nintendo zaprezentowała nową konsolę wykorzystującą właśnie to rozwiązanie. Nintendo~3DS było wyposażone w dwie kamery, które umożliwiały robienie zdjęć 3D \cite{bib:Ito2012Stereoscopic}. Niestety, to rozwiązanie wymaga znajomości parametrów kamer i ich wzajemnego położenia, a także kalibracji układu.

Ciekawym podejściem do omawianego problemu jest aktywny pomiar odległości, w którym urządzenie emituje sygnał i wyznacza odległość na bazie jego powrotu. Jednym z takich urządzeń jest LiDAR (ang. \english{Light Detection and Ranging}), który emituje promienie lasera i analizuje sygnał odbity od obiektów. Jeżeli dane rozwiązanie wykorzystuje metodę czasu przelotu, to odległość jest wyznaczana na podstawie czasu między emisją impulsu, a zarejestrowaniem jego powrotu.
...
Zależnie od źródła danych, do poszczególnych punktów mogą zostać dodane dodatkowe informacje, np. intensywność sygnału, kolor lub wektor normalny. Chmura punktów, przeciwnie do obrazu RGB, nie musi tworzyć regularnej siatki i zazwyczaj traktuje się ja jak nieuporządkowany zbiór punktów. Kolejność zbioru nie powinna mieć znaczenia dla reprezentacji geometrii obiektu \cite{bib:qi2017pointnet}.

Brak regularnej struktury chmury punktów powoduje, że jej przetwarzanie nie może być zrealizowane w identyczny sposób jak obrazu. Z tego powodu zaproponowano wiele architektur służących wyłącznie do ich przetwarzania. Jedną z nich jest PointNet który, aby przetworzyć chmurę punktów, korzysta z cech poszczególnych punktów, a następnie wykonuje na nich symetryczną agregację. Dzięki temu wynik nie jest zależny od kolejności punktów wejściowych. Ta architektura jest wykorzystywana m.in. do klasyfikacji i segmentacji obiektów trójwymiarowych \cite{bib:qi2017pointnet}.

Podstawowa wersja PointNet tylko w małym stopniu uwzględnia lokalne zależności między punktami, co poskutkowało powstaniem jej rozwinięcia: PointNet++. Wprowadzono w nim hierarchiczne grupowanie punktów oraz uczenie cech dla kolejnych obszarów przestrzennych. Umożliwia to analizę geometrii dla różnych poziomów szczegółowości oraz lepsze uwzględnienie zmiennej gęstości próbek, która występuje w chmurach punktów \cite{bib:qi2017pointnetplusplus}.

Chmury punktów mogą być wykorzystywane bezpośrednio do detekcji obiektów, czego przykładem jest VoteNet. Jego architektura oparta jest na PointNet++ oraz mechanizmie głosowania Hougha. Kolejne punkty wraz z ich cechami generują głosy wskazujące środki potencjalnych obiektów, następnie są grupowane, a na ich podstawie wyznacza się trójwymiarowe ramki ograniczające. To rozwiązanie nie wymaga wiedzy o kolorach obiektów, a bazuje jedynie na chmurze, bez potrzeby konwertowania jej do regularnej reprezentacji przestrzennej \cite{bib:ding2019votenet}.

Zdolność detekcji obiektów w chmurze punktów nie jest niestety nieograniczona. Wiele zależy od jakości pomiarów dostarczanych przez skaner i metody ich analizy. Rzeczywiste pomiary mogą zawierać w sobie losowe szumy, odstające punkty, czy zmienną gęstość próbek. Ten problem uwidacznia się szczególnie przy analizie niewielkich elementów geometrii, które przy chmurze małej gęstości mogą być reprezentowane przez pojedyncze punkty. Jak wskazują Zhang et al., takie zakłócenia obecne w rzeczywistych chmurach punktów mogą prowadzić pogorszenia skuteczności rozpoznawania obiektów. Odpowiedzią autorów na ten problem jest architektura R-PointNet, która zwiększa odporność modelu na zakłócenia \cite{bib:zhang2024rpointnet}.

Dane pochodzące z LiDAR są przede wszystkim opisem geometrii obserwowanej sceny i nie przenoszą  informacji o kolorze lub teksturze, które dostępne są w obrazach z kamery. Jednakże, istnieją metody dołączania tych danych przy pomocy dodatkowej kamery. Możliwości każdej z tych reprezentacji niejako uzupełniają wzajemnie swoje ograniczenia. Możliwe jest więc ich jednoczesne wykorzystanie, by wspomóc proces detekcji wykorzystując zarówno dane wizualne, jak i przestrzenne.
...
Połączenie ze sobą danych wizualnych i przestrzennych sprawia, że otrzymujemy wspólną reprezentację sceny. Obraz z kamery dostarcza informacji o kolorze i krawędziach, a mapy głębi i chmury punktów dają nam możliwość bezpośredniego umiejscowienia punktów w przestrzeni, ukazując geometrię przestrzenną całej sceny. Ograniczenia każdej z tych modalności skłaniają naukowców do dywagacji na temat możliwości ich wspólnego użycia. W literaturze znajdujemy wiele metod fuzji danych. Celem tego działania jest uzyskanie reprezentacji, która będzie zawierała w sobie zarówno cechy wizualne, jak i przestrzenne obserwowanych przedmiotów. Takie podejście zastosowano w modelu ImVoteNet, będącym rozwinięciem architektury VoteNet. Model łączy ze sobą informacje z obrazów RGB z cechami pochodzącymi z chmury punktów. Na obrazie wyznaczane są cechy geometryczne, semantyczne i teksturowe, a następnie są one przenoszone do przestrzeni trójwymiarowej. Po połączeniu ich z cechami punktów następuje głosowanie, na wzór modelu VoteNet. Na zbiorze SUN RGB-D model osiągnął wartość $mAP@0.25$ wynoszącą $63.4$, poprawiając wynik swego poprzednika o $5.7$ punktu \cite{bib:qi2020imvotenet}.

Korzyści z łączenia danych wykazali również Liu et al. proponując model, który wykonywał równolegle detekcje na obrazie z kamery oraz mapie głębi z LiDAR. Ich rozwiązanie, łączące dane, uzyskało  lepsze wyniki niż detekcje wykonywane osobno na obu modalnościach. Podczas eksperymentów wartość mAP modelu osiągnęła wynik $89.26\%$, wyprzedzając warianty wykorzystujące pojedyncze źródło danych o $2.56$ punktu procentowego dla modelu korzystającego obrazy i $14.99$ punktu procentowego dla modelu korzystającego z danych z LiDAR. Jednocześnie wskazali, że przetworzenie pojedynczej klatki zajmowało jedynie $0.03$ sekundy, co teoretycznie umożliwia działanie modelu w czasie rzeczywistym \cite{bib:liu2023lidarcamerafusion}.

Architektura rodzina YOLO może również stanowić element systemów wykorzystujących fuzję danych. Przykładami mogą być sieć \nolinebreak{YOLO-POINT}, w której autorzy połączyli detektor YOLOv5 z siecią PointNet++, a także sieć \nolinebreak{DTC-YOLO}, która wykorzystuje jednocześnie obraz RGB oraz LiDAR. W \nolinebreak{DTC-YOLO} zastosowano odwzorowanie informacji o głębi na obraz i ważoną fuzję obu reprezentacji. Model wykorzystuje też sieć \nolinebreak{ADF\textsuperscript{3}-Net}, która wykorzystuje dynamiczne bramkowanie do łączenia cech pochodzących z różnych poziomów. Eksperymenty z jej zastosowaniem na zbiorze KITTI dały wynik $mAP50$ równy $0.911$ oraz $mAP50\text{-}95$ równą $0.693$, co przewyższa wyniki osiągane przez porównywane modele YOLOv8n, YOLOv10n oraz YOLO11n \cite{bib:ma2024yolopoint,bib:xu2025dtcyolo}.

Wskazuje to, że połączenie danych z różnorodnych modalności może poprawić skuteczność detekcji, a jednocześnie nie musi uniemożliwiać działania w czasie rzeczywistym. Największą wadą, jaką należy wziąć pod uwagę, jest to, że aby wykorzystać bezpośrednio mierzone informacje o głębi konieczne jest zastosowanie dodatkowego sensora np. LiDAR. Korzystanie z kilku źródeł danych może również wymagać ich kalibracji i synchronizacji. Przetwarzanie dużego wolumenu danych może zwiększyć znacząco wymagania obliczeniowe systemu. Przydatność fuzji należy więc rozpatrywać nie tylko z perspektywy potencjalnego wzrostu jakości detekcji, ale także z perspektywy dostępności odpowiednich sensorów, jakości pozyskiwanych danych oraz wymagań dotyczących czasu działania systemu.
...
\draft{
	W urządzeniach Apple wyposażonych w skaner LiDAR framework ARKit
	umożliwia dostęp do mapy głębi sceny poprzez mechanizm
	\texttt{sceneDepth}. Każdy element mapy odpowiada oszacowanej
	odległości fragmentu obserwowanej sceny od płaszczyzny kamery.
	Apple wskazuje jednak, że mapa głębi udostępniana przez ARKit ma
	znacznie niższą rozdzielczość niż odpowiadający jej obraz RGB
	\cite{bib:appleSceneDepth}.
}
