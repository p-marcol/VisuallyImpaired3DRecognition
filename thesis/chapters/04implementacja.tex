...
Analiza wykonywana jest używając klasy \texttt{AnalysisWorker}. Wykorzystuje ona osobny wątek w obiekcie \texttt{ThreadPoolExecutor}, dzięki czemu może wykonywać obliczenia poza główną pętlą zdarzeń serwera. Klasa na podstawie wyniku detekcji w kontekście wyodrębnia fragment obrazu ograniczony ramką detekcji. Jeżeli snapshot nie istnieje, model nie wykrył obiektu lub wycinek jest niepoprawny, dalsza analiza nie jest wykonywana.
...
Rozpoznanie widocznej powierzchni odbywa się w module \texttt{SurfaceMacher}. Do każdej powierzchni w bazie wiedzy dopasowywany jest kolor referencyjny zapisany jako nazwa koloru CSS oraz komunikat informacyjny. Nazwa koloru jest przekształcana do przestrzeni RGB, a następnie do układu BGR i przestrzeni Lab. Wycinek obrazu również jest przekształcany do przestrzeni Lab, a następnie wykonywane jest dopasowanie kolorów. Odbywa się ono dla każdego piksela wycinka poprzez obliczenie kwadratu odległości euklidesowej od kolorów referencyjnych wszystkich powierzchni. Piksel jest przypisywany do koloru wyłącznie wtedy, gdy uzyskana odległość jest mniejsza od ustalonego progu \texttt{max\_distance} znajdujacego się w konfiguracji bryły. Następnie zliczana jest ilość pikseli należących do każdej z powierzchni. Wynikiem jest informacja o powierzchni, której kolor został dopasowany do największej liczby pikseli wycinka. 
...
Obsługa żądania informacji przewiduje możliwość wystąpienia wyjątku przy wykonywaniu analizy wycinka. Błąd nie zatrzymuje działania serwera, a skutkuje przygotowaniem komunikatu zastępczego. Wyjątki związane bezpośrednio z odczytem bazy wiedzy i dopasowaniem powierzchni są obsługiwane w module analizy i również prowadzą do przygotowania komunikatu zastępczego. Nieprzewidziane wyjątki w trakcie zadania analizy są odnotowane w logach serwera, ale do klienta nie jest wysyłany żaden komunikat o wystąpieniu błędu. 
