...
Pełen zbiór danych wraz z adnotacjami zawiera 9332 obrazów i adnotacji w formacie YOLO oraz 4 pliki tekstowe. Zajmuje on łącznie $3,19$ GB.
...
\end{itemize}
Łącznie wytrenowano 8 modeli, które pozwoliły na przeprowadzenie eksperymentów i porównanie jakości detekcji pomiędzy nimi.

Urządzenia wykorzystane do treningu były wybierane według dostępności w danym momencie, a nie według ich wydajności. Trening na karcie graficznej NVIDIA RTX 4060 był wykonywany na lokalnym komputerze z systemem Windows~11, a treningi na pozostałych kartach odbywały się w chmurze za pomocą platformy RunPod z systemem Ubuntu~22.04. Wybór urządzenia i systemu operacyjnego miał wpływ jedynie na czas treningu, a nie na jakość detekcji.
...
Każdy trening uruchomiony został z ustawieniem ziarna generatora liczb pseudolosowych na wartość 42. Zapewnia to powtarzalność wyników w identycznych warunkach, a także umożliwia odtworzenie wyników w przyszłości. Wartość ta została wybrana ze względu na jej popularność w literaturze i kulturze komputerowej\footnote{Wartość 42 jest nawiązaniem do powieści Douglasa Adamsa \textit{Autostopem przez Galaktykę}, gdzie stanowi odpowiedź na  "Wielkie Pytanie o Życie, Wszechświat i całą resztę"}.

Wartości pozostałych parametrów treningu zostały przyjęte jako domyślne w bibliotece Ultralytics i nie były modyfikowane. Do treningu wykorzystano następujące wersje bibliotek i narzędzi:
\begin{itemize}
...
Nie porównywano czasu inferencji, zużycia pamięci ani liczby możliwych do przetworzenia klatek na sekundę, ponieważ są to wartości silnie zależne od używanego urządzenia obliczeniowego, przepustowości sieci oraz obciążenia systemu operacyjnego. Każdy przypadek wdrożenia systemu należy oceniać indywidualnie, a nie na podstawie wyników eksperymentów. Wartości te nie wpływają na ocenę jakości detekcji, a jedynie na kulturę pracy systemu.
...
\begin{enumerate}[label=\textbf{Eksperyment \Alph*:}, leftmargin=*, align=left]
    ...
    \item Porównanie jakości detekcji pomiędzy wariantami danych: kolorowym i w odcieniach szarości. Pozwoli to ocenić, czy usunięcie informacji o barwie wpływa na jakość detekcji i czy model jest w stanie rozpoznać obiekty wyłącznie na podstawie ich kształtu.
    \item Porównanie jakości detekcji pomiędzy najlepszymi modelami z poprzednich eksperymentów. Umożliwi to ocenę, który z badanych wariantów modelu jest najlepszy.
\end{enumerate}
